...
# NKI encoder megakernel (optional — falls back to per-layer forward if not available)
try:
    from neuronx_distributed_inference.models.whisper.whisper_encoder_megakernel import (
...
def _unpack_layer_weights(packed: Tensor, layer_idx: int):
    """Unpack the 13 weight tensors for a single layer from the packed tensor.
    ...
    Returns:
        tuple of 13 tensors in canonical order (attn_ln_w, attn_ln_b, qkv_w, ...)
    """
...
class NeuronApplicationWhisperEncoder(NeuronApplicationBase):
    ...
    def _build_packed_weights(self):
        ...
        # The neuron state dict uses keys like:
        #   blocks.{i}.attn_ln.weight, blocks.{i}.attn_ln.bias
        #   blocks.{i}.attn.qkv_proj.weight, blocks.{i}.attn.qkv_proj.bias
        #   blocks.{i}.attn.out.weight, blocks.{i}.attn.out.bias
        #   blocks.{i}.mlp_ln.weight, blocks.{i}.mlp_ln.bias
        #   blocks.{i}.mlp.up_proj.weight, blocks.{i}.mlp.up_proj.bias
        #   blocks.{i}.mlp.down_proj.weight, blocks.{i}.mlp.down_proj.bias
        dtype = torch.bfloat16
        n_layers = self.config.dims.n_audio_layer
    ...
    def forward(self, audio: torch.Tensor) -> torch.Tensor:
        ...
        if self._use_megakernel and self._packed_weights is not None:
            ...
            # Non-megakernel path: just audio input
