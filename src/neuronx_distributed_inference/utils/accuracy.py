...
from neuronx_distributed_inference.models.llama4.utils.input_processor import (
    prepare_generation_inputs_hf as llama4_prepare_generation_inputs_hf,
)
...
from neuronx_distributed_inference.utils.version_utils import (
...
def check_accuracy_embeddings(
    ...
    passed, max_err = custom_allclose(
        expected_output, actual_output, atol=atol, rtol=rtol
    )
...
def get_generate_outputs_from_token_ids(
    ...
    **generate_kwargs,
    ...
    generation_model = (
        model if is_hf else HuggingFaceGenerationAdapter(model, input_start_offsets)
    )
    token_ids = _shift_tensors_by_offset(
        input_start_offsets, token_ids, tokenizer.pad_token_id
    )
...
def check_accuracy(
    ...
    mode_being_tested = (
        "async mode" if neuron_model.neuron_config.async_mode else "sync mode"
    )
    ...
    pad_token_id = (
        tokenizer.convert_tokens_to_ids(tokenizer.pad_token) if tokenizer else 0
    )
    ...
    if draft_model is not None or neuron_config.enable_fused_speculation:
        ...
        tokens_to_compare = min(
            expected_token_ids.shape[-1], output_token_ids.shape[-1]
        )
        expected_token_ids = expected_token_ids[:tokens_to_compare]
        output_token_ids = output_token_ids[:tokens_to_compare]

    device = "neuron"
    assert torch.equal(output_token_ids, expected_token_ids), (
        f"\nActual: ({device}) {output_token_ids} \nExpected (hf-cpu): {expected_token_ids}"
    )
    logger.info(
        f"The output from Neuronx NxD on {device} using {mode_being_tested} is accurate!"
    )
...
def prepare_inputs_from_prompt(
    ...
    if neuron_model.config.neuron_config.is_chunked_prefill:
        ...
        prompts = [
            prompt
        ] * neuron_model.config.neuron_config.chunked_prefill_config.max_num_seqs
    else:
...
def generate_expected_logits(
    ...
    if isinstance(neuron_model, NeuronMllamaForCausalLM):
        raise ValueError(
            "Mllama does not support generating goldens with HF on CPU. Please generate logits separately."
        )

    if neuron_model.neuron_config.enable_fused_speculation:
        generation_config.prompt_lookup_num_tokens = (
            neuron_model.neuron_config.speculation_length
        )
    ...
    logger.info(
        f"Expected Output (token ids with the max logits): {expected_token_ids}"
    )
...
def check_accuracy_logits(
    ...
    if neuron_model.neuron_config.on_device_sampling_config is not None:
        ...
        assert neuron_model.neuron_config.output_logits, (
    ...
    initial_input_ids = _shift_tensors_by_offset(
        input_start_offsets, initial_input_ids, pad_token_id
    )
    initial_attention_mask = _shift_tensors_by_offset(
        input_start_offsets, initial_attention_mask, 0
    )
    ...
    if isinstance(neuron_model, NeuronMllamaForCausalLM):
        neuron_vision_input_args, hf_vision_input_args = _prepare_mllama_vision_args(
            neuron_model,
            tokenizer,
            prompt,
            image,
        ...
        neuron_vision_input_args, hf_vision_input_args = _prepare_llama4_vision_args(
            neuron_model,
            prompt,
    ...
    model = HuggingFaceGenerationAdapter(
        neuron_model, input_start_offsets=input_start_offsets
    )
    ...
    def generate_fn_base(input_ids, divergence_idx=None):
        ...
        if (
            "input_ids" in neuron_vision_input_args
            and "attention_mask" in neuron_vision_input_args
        ):
        ...
        if isinstance(neuron_model, NeuronLlama4ForCausalLM) or isinstance(
            neuron_model, NeuronLlama4TextForCausalLM
        ):
        ...
        if (
            ...
            and get_torch_neuronx_build_version()
            >= packaging.version.parse("2.11.14773")
        ):
...
def check_accuracy_logits_v2(
    ...
    max_new_tokens = (
        neuron_model.config.neuron_config.seq_len - inputs_input_ids.shape[1]
    )
    if num_tokens_to_check is None:
        num_tokens_to_check = max_new_tokens
    else:
    ...
    def generate_fn_base(input_ids):
        ...
        if (
            isinstance(neuron_model, NeuronLlama4ForCausalLM)
            and actual_logits.shape[-1] > expected_logits.shape[-1]
        ):
        ...
        logger.info(
            f"Actual Output (token ids with the max logits): {actual_token_ids}"
        )
        logger.info(f"Actual Logits Shape: {actual_logits.shape}")
        if (
            neuron_model.neuron_config.on_device_sampling_config is not None
...
def shift_inputs_by_offset(inputs, input_start_offsets, pad_token_id=0):
    ...
    """
    shifted_input_ids = _shift_tensors_by_offset(
        input_start_offsets, inputs.input_ids, pad_token_id
    )
    shifted_attention_mask = _shift_tensors_by_offset(
        input_start_offsets, inputs.attention_mask, 0
    )
...
def _shift_tensors_by_offset(input_start_offsets, input_tensors, pad_token_id):
    ...
    if input_start_offsets:
        ...
        if len(input_start_offsets) > 1:
            ...
            new_token_ids[:, offset : offset + seq_len] = (
                input_tensors  # if there is only one offset value, shift all sequences the same amount
            )
...
def generate_with_chunked_prefill(
    ...
    block_table = torch.arange(max_num_seqs * num_blocks_per_seq).reshape(
        max_num_seqs, -1
    )
...
def _prepare_mllama_vision_args(
    neuron_model,
    tokenizer,
    prompt,
    image,
    ...
    if not hasattr(inputs, "pixel_values"):
        if image is None:
            raise ValueError(
                "Image input is required to check logit accuracy for a Mllama model."
            )
        if tokenizer is None:
            raise ValueError(
                "A tokenizer is required to check logit accuracy for a Mllama model."
            )

        # Llama3.2 vision inputs
        batch_image = [[image] * 1] * neuron_model.config.neuron_config.batch_size
        ...
        if image_processor is not None:
            hf_vision_input_args = image_processor(
...
def _prepare_llama4_vision_args(neuron_model, prompt, image, inputs, image_processor):
    ...
    if prompt is not None:
        ...
        input_ids, attention_mask, pixel_values, vision_mask = (
            llama4_prepare_generation_inputs_hf(
                prompt, image, image_processor, role="user", config=neuron_model.config
            )
        )
        neuron_vision_input_args = {
        ...
        if (
            inputs is None
            or getattr(inputs, "pixel_values", None) is None
            or getattr(inputs, "vision_mask", None) is None
        ):
            ...
            _, _, pixel_values, vision_mask = llama4_prepare_generation_inputs_hf(
                prompt, image, image_processor, role="user", config=neuron_model.config
            )
        else:
            pixel_values = inputs.pixel_values
...
def _prepare_pixtral_vision_args(neuron_model, prompt, image, inputs, image_processor):
    ...
    if prompt is not None:
        ...
        input_ids, attention_mask, pixel_values, vision_mask, image_sizes = (
            pixtral_prepare_generation_inputs_hf(
                prompt, image, image_processor, role="user", config=neuron_model.config
            )
        )
        input_ids, attention_mask, pixel_values, vision_mask, image_sizes = (
        ...
        if (
            ...
            _, _, pixel_values, vision_mask, image_sizes = (
                pixtral_prepare_generation_inputs_hf(
                    prompt,
                    image,
                    image_processor,
...
def run_accuracy_draft_logit_test_flow(
    model: NeuronApplicationBase,
    generation_config: GenerationConfig,
    tokenizer: PreTrainedTokenizer,
    draft_golden_path: str,
    num_draft_loops_to_check: int,
):
    assert model.config.neuron_config.output_logits, (
        "output_logits hos to be enabled to use this functionality"
    )
    assert num_draft_loops_to_check < MAX_DRAFT_LOOP_TO_CHECK, (
        f"We only support checking accuracy up to {MAX_DRAFT_LOOP_TO_CHECK} draft loops"
    )
    generation_config.do_sample = False
    generation_config.prompt_lookup_num_tokens = (
        model.config.neuron_config.speculation_length
    )
    ...
    check_accuracy_draft_logit(
        model_outputs.scores, draft_golden_path, num_draft_loops_to_check
    )


def check_accuracy_draft_logit(
    actual: List[torch.Tensor],
    draft_golden_path: str,
    num_draft_loops_to_check: int = 6,
) -> Optional[dict]:
    ...
    for i, file_number in zip(
        range(1, num_draft_loops_to_check + 1), draft_golden_file_numbers
    ):
        ...
        result, test_pass, draft_iter = check_logits_per_draft_loop(
            actual[i], draft_logits, i
        )
        assert test_pass, (
            f"Draft logit validation fails in draft_loop {i} during draft iteration {draft_iter}. Top k logit result: {result}."
        )
...
def get_sorted_draft_file_number_list(draft_goldens_dir: str) -> List[str]:
    ...
    return sorted(
        [
            int(re.search(r"draft_logits_(\d+)\.pt", f).group(1))
            for f in file_list
            if re.match(r"draft_logits_\d+\.pt$", f)
        ]
    )


def check_logits_per_draft_loop(
    actual: torch.Tensor, expected: torch.Tensor, draft_loop: int
) -> Tuple[AllCloseSummary, bool]:
    ...
    for i, expected_token_id in enumerate(expected_draft_token_ids):
        ...
        if actual_token_id != expected_token_id:
            ...
            print(
                f"Draft token ids diverge! Neuron: {actual_token_id} - Expected: {expected_token_id}, we can only validate up to draft iter {i} at draft loop {draft_loop}"
            )
            return results, test_pass, i
    return results, test_pass, i


def check_draft_logits(
    actual: torch.Tensor, expected: torch.Tensor, top_k: int
) -> AllCloseSummary:
