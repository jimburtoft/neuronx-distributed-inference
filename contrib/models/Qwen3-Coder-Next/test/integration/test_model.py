#!/usr/bin/env python3
"""
Integration tests for Qwen3-Coder-Next NeuronX implementation.
...
Accuracy Validation Approach:
  ...
  against pre-verified CPU reference outputs. This validates that the model
  produces correct outputs end-to-end.

  Note on logit-level validation:
  logit_validation() is not used here. Qwen3-Coder-Next has a hybrid DeltaNet
  ...
  tokens, producing logit divergences that exceed standard tolerances.

Environment:
  - trn2.3xlarge with Neuron SDK 2.30
...
COMPILED_MODEL_PATH = os.environ.get(
    "COMPILED_PATH", "/mnt/models/compiled_qwen3_coder_next/"
)
...
def create_config(model_path: str):
    ...
    # SDK 2.30 config: use shard_on_block for CTE path
    # (shard_hidden kernel broken on SDK 2.30; block_size=128 with PING_PONG works)
    ...
    config_dict["pad_token_id"] = text_config.get("eos_token_id", 151645)
