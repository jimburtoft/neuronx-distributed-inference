    ...
    def input_generator(
        ...
        for bucket in self.neuron_config.buckets:
            ...
            if self.neuron_config.enable_fused_speculation:
                # Fused speculation flow. The traced module is NeuronFusedSpecModel,
                # whose forward positional layout is:
                ...
                # For the context-encoding tag we trace with the full prefill
                # n_active_tokens and REAL (dummy-filled) vision inputs so the
                # audio/image embeddings scatter into inputs_embeds for both the
                # target and draft. For the fused-speculation (token-gen) tag,
                # vision inputs are empty because the audio is already resident in
                # the KV cache from prefill.
                if self.tag == FUSED_SPECULATION_MODEL_TAG:
                    # enable_fused_spec() sets n_active_tokens=1: the fused NEFF
                    # takes a single input token and internally runs the K draft
                    # steps + target verify. Match the plain ModelWrapper fused-spec
                    # convention (input_ids/position_ids are [batch, n_active_tokens]).
                    fs_active_tokens = (
                        bucket
                        if self.neuron_config.bucket_n_active_tokens
                        else self.neuron_config.n_active_tokens
                    )
                    fs_input_ids = torch.zeros(
                        (self.neuron_config.batch_size, fs_active_tokens), dtype=torch.int32
                    )
                    fs_position_ids = torch.zeros(
                        (self.neuron_config.batch_size, fs_active_tokens), dtype=torch.int32
                    )
                    fs_vision_embeddings = torch.zeros((0), dtype=self.config.neuron_config.torch_dtype)
                    fs_vision_mask = torch.zeros((0), dtype=torch.bool)
                    inputs.append(
                        (
                            fs_input_ids,
                            attention_mask,
                            ...
                            fs_vision_embeddings,
                            fs_vision_mask,
                        )
                    )
                else:  # CONTEXT_ENCODING_MODEL_TAG with fused speculation
                    inputs.append(
                        (
                            input_ids,
                            attention_mask,
            ...
            elif self.tag == CONTEXT_ENCODING_MODEL_TAG or self.tag == TOKEN_GENERATION_MODEL_TAG:
                inputs.append(
                    (
                        input_ids,
                        attention_mask,
