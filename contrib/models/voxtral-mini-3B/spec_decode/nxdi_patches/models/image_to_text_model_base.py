...
from neuronx_distributed_inference.models.model_base import NeuronBaseForCausalLM, NeuronFusedSpecModel
...
class NeuronBaseForImageToText(NeuronBaseForCausalLM):
    def __init__(
        ...
        **kwargs,
        ...
        # Propagate the fused_spec_config from the outer (multimodal) config down
        # to the text_config, since below we operate with self.config = text_config
        # and NeuronFusedSpecModel reads fused_spec_config from the config it is
        # given.
        if self.neuron_config.enable_fused_speculation:
            self.config.fused_spec_config = self.full_config.fused_spec_config
        ...
        if self.neuron_config.enable_fused_speculation:
            # Fused speculation for image-to-text models. Mirror the flow in
            # NeuronBaseForCausalLM.__init__, but keep self.model_wrapper as the
            # image-to-text wrapper so that vision/audio embeddings are traced
            # into both the CTE and the fused-spec NEFF. The target and draft
            # sub-models (inside NeuronFusedSpecModel) both receive the modality
            # embeddings during prefill so their KV caches are conditioned on the
            # audio/image context.
    ...
    def get_text_builder(self, debug=False):
        if self.text_builder is None:
            ...
            self.text_builder = ModelBuilder(
                ...
                init_custom_process_group_fn=(
                    partial(init_custom_process_group_fn, self.full_config)
                    if self.neuron_config.enable_fused_speculation
                    else None
                ),
    ...
    def _get_model_outputs(
        ...
        if self.neuron_config.enable_fused_speculation:
            # Fused speculation flow. Both the context-encoding NEFF and the
            # fused-speculation NEFF are backed by NeuronFusedSpecModel, whose
            # forward positional layout is:
            ...
            if self._is_prefill(position_ids):
                outputs = self.context_encoding_model(
                    ...
                    vision_embeddings,  # real audio/image embeds -> scatter for target+draft
                    vision_mask,
                )
                ...
                outputs = self.fused_spec_model(
                    ...
                    torch.empty(0),  # num_queries
                    torch.empty(0),  # computed_context_lens
                    torch.empty(0),  # vision_embeddings (empty: audio already in KV)
