    ...
    def _forward(self, *args):
        ...
        if self.pipeline_execution:
            ...
            if self.return_ranked_to_cpu:
                # return logits if on_device_sampling enabled and output_logits=True
                if self.neuron_config.on_device_sampling_config and self.neuron_config.output_logits:
                    return ranked_output[0][0].to('cpu'), ranked_output[0][1].to('cpu')
    ...
    def pad_inputs(self, *args, pad_type="first_fit"):
        ...
        if (self.tag == CONTEXT_ENCODING_MODEL_TAG or self.tag == VISION_ENCODER_MODEL_TAG):
            ...
            if len(args) >= 24 and len(args[23].shape) == 3 and args[23].shape[1] != pad_length:
                ...
                logger.warning(
                    f"Vision mask shape {args[23].shape} does not match padded sequence length {pad_length}. "
                    f"Replacing vision_embeddings and vision_mask with dummy inputs. "
                    f"To provide real vision data, ensure vision tensors have sequence dimension == {pad_length} "
                    f"(the CTE bucket size, set by enable_context_encoding())."
                )
