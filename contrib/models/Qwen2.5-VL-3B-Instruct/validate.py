#!/usr/bin/env python3
"""
Validation script for Qwen2.5-VL-3B-Instruct NeuronX contrib.
...
Usage:
    # Full validation (compile + inference + comparison):
    python validate.py --model-path /home/ubuntu/models/Qwen2.5-VL-3B-Instruct

    # Skip compilation if already compiled:
    python validate.py --model-path /home/ubuntu/models/Qwen2.5-VL-3B-Instruct --skip-compile

    # Inference-only (no HF comparison):
    python validate.py --model-path /home/ubuntu/models/Qwen2.5-VL-3B-Instruct --skip-reference
"""
...
def compile_neuron_model(model_path: str, compiled_path: str, tp_degree: int = 2):
    ...
    config = Qwen2_5_VL3BInferenceConfig(
        neuron_config,
        load_config=load_pretrained_config(model_path),
    )

    model = NeuronQwen2_5_VL3BForCausalLM(model_path, config)
...
def main():
    parser = argparse.ArgumentParser(
        description="Validate Qwen2.5-VL-3B-Instruct on Neuron"
    )
    parser.add_argument("--model-path", required=True, help="Path to HF model weights")
    parser.add_argument(
        "--compiled-path",
        default="/home/ubuntu/neuron_models/Qwen2.5-VL-3B-Instruct/",
    ...
    print(f"{'=' * 60}")
    print(f"Qwen2.5-VL-3B-Instruct Validation")
    print(f"{'=' * 60}")
    ...
    # Step 2: Load and run Neuron inference
    ...
    # Clean up Neuron model before loading HF (memory)
    del model
    gc.collect()

    # Step 3: HF reference comparison
