#!/bin/bash
# MiniMax M2.5 BF16 - 8K config
# max-model-len=8480, max-num-seqs=64, batch_size=64
# context_encoding=[8192], token_generation=[8480]
# Input: 7400, Output: 300
...
python3 -m vllm.entrypoints.openai.api_server \
    ...
    --additional-config '{
        "override_neuron_config": {
            ...
            "ctx_batch_size": 1,
            "tkg_batch_size": 64,
            "max_context_length": 8480,
            "seq_len": 8480,
            "async_mode": true,
            "flash_decoding_enabled": false,
            "enable_bucketing": true,
            "context_encoding_buckets": [8192],
            "token_generation_buckets": [8480],
